...
import { Send, Smile, ArrowLeft, Users, MoreVertical, Hash } from 'lucide-react';
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <div className="h-14 flex items-center gap-3 px-4 border-b border-white/5 glass flex-shrink-0">
          <Link href={`/groups/${id}`} className="text-text-muted hover:text-text-secondary transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${group?.avatar_color || '#6366f1'}20`, color: group?.avatar_color || '#6366f1' }}>
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${showMembers ? 'bg-brand/15 text-brand-light' : 'text-text-muted hover:text-text-secondary hover:bg-white/5'}`}
          >
          ...
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center py-20">
              <div className="w-14 h-14 rounded-2xl bg-brand/10 border border-brand/20 flex items-center justify-center mb-4">
                <Hash className="w-7 h-7 text-brand-light" />
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {group_msg.messages.map((msg, mi) => (
                  <div key={msg.id || mi} className="relative">
                    <div className={`inline-block max-w-[80%] px-3 py-2 rounded-xl text-sm text-text-primary mb-1 ${
                    ...
                    {Object.keys(msg.reactions || {}).length > 0 && (
                      <div className="flex gap-1 mt-0.5 flex-wrap">
                        {Object.entries(msg.reactions).map(([emoji, users]) => (
                          <span key={emoji} className="glass rounded-full px-2 py-0.5 text-xs flex items-center gap-1 cursor-pointer hover:bg-white/10 transition-colors">
        ...
        <div className="p-4 border-t border-white/5 flex-shrink-0">
          <div className="flex items-end gap-2 glass rounded-2xl border border-white/10 p-2">
            <div className="relative">
              <button onClick={() => setShowEmoji(!showEmoji)} className="w-8 h-8 flex items-center justify-center text-text-muted hover:text-text-secondary transition-colors rounded-lg hover:bg-white/5">
              ...
              <AnimatePresence>
                {showEmoji && (
                  <motion.div
                    ...
                    className="absolute bottom-10 left-0 glass rounded-xl border border-white/10 p-2 flex gap-1 shadow-card z-10"
                  >
                    {REACTIONS.map(e => (
            ...
            <textarea
              ...
              rows={1}
              className="flex-1 bg-transparent text-sm text-text-primary placeholder:text-text-muted resize-none focus:outline-none py-1.5 max-h-32"
              style={{ fieldSizing: 'content' } as React.CSSProperties}
            />
            <button
              onClick={handleSend}
              disabled={!input.trim()}
              className="w-8 h-8 rounded-xl bg-brand flex items-center justify-center text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:bg-brand-dark active:scale-95 flex-shrink-0"
            >
      ...
      <AnimatePresence>
        {showMembers && (
          <motion.div
            ...
            className="glass border-l border-white/5 overflow-hidden flex-shrink-0"
          >
            <div className="p-4">
              <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Members</h3>
                ...
                {members?.map((m: { id: number; user: { id: number; first_name: string; last_name: string; avatar: string | null }; role: string }) => (
                  <div key={m.id} className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {m.role === 'admin' && <div className="text-[10px] text-accent-amber">Admin</div>}
                    </div>
                  </div>
                ))}
              </div>
